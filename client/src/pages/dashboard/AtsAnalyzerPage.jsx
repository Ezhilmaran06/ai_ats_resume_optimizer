...
export default function AtsAnalyzerPage() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
      ...
      {/* Results View */}
      {report && (() => {
        const scoreVal = report.overallScore ?? report.score ?? 0;
        const displayScore = report.displayScore || `${scoreVal} / 100`;
        const scoreLabel = report.label || 'ATS Compatibility Score';

        // 13 dimensions from Python FastAPI or 7 fallback categories
        const rubricList = report.categoryScores
          ? Object.values(report.categoryScores).map(c => ({
              label: c.name,
              val: c.score,
              max: c.maxScore || 8,
              feedback: c.feedback,
          ...
          : [
              { label: 'Keyword Relevance', val: report.categories?.keywordRelevance ?? 18, max: 25 },
              { label: 'Technical Skills Match', val: report.categories?.skillsMatch ?? 16, max: 20 },
              { label: 'Job Relevance Alignment', val: report.categories?.jobRelevance ?? 12, max: 15 },
              { label: 'Standard Resume Structure', val: report.categories?.structure ?? 8, max: 10 },
              { label: 'Section Completeness', val: report.categories?.sectionCompleteness ?? 8, max: 10 },
              { label: 'Readability & Action Verbs', val: report.categories?.readability ?? 8, max: 10 },
              { label: 'Formatting & Parseability', val: report.categories?.formatting ?? 8, max: 10 }
            ];
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div className="card" style={{ display: 'grid', gridTemplateColumns: '260px 1fr', gap: '32px', alignItems: 'center' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ position: 'relative', width: '160px', height: '160px' }}>
                  <svg width="160" height="160" viewBox="0 0 160 160">
                    ...
                    <circle
                      ...
                      cy="80"
                      r="68"
                      stroke={scoreVal >= 80 ? '#10B981' : (scoreVal >= 65 ? '#2563EB' : '#F59E0B')}
                      strokeWidth="12"
                      fill="none"
                      strokeDasharray={427}
                      strokeDashoffset={427 - (427 * scoreVal) / 100}
                      strokeLinecap="round"
                ...
                <span className="badge badge-success" style={{ marginTop: '12px', fontWeight: '700' }}>
                  {scoreLabel}: {displayScore}
                </span>
                <span style={{ fontSize: '12px', color: 'var(--text-muted)', marginTop: '4px' }}>
                  {scoreVal >= 80 ? 'High ATS Compatibility' : (scoreVal >= 65 ? 'Moderate Compatibility' : 'Action Required')}
                </span>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '4px' }}>
                  <h3 style={{ fontSize: '16px', fontWeight: '700' }}>ATS Category Diagnostic</h3>
                  <span style={{ fontSize: '12px', color: 'var(--text-muted)' }}>
                    {rubricList.length} Evaluation Dimensions
                  </span>
                </div>

                <div style={{
                  display: 'grid',
                  gridTemplateColumns: rubricList.length > 8 ? '1fr 1fr' : '1fr',
                  gap: '8px 20px'
                }}>
                  {rubricList.map(cat => (
                    <div key={cat.label}>
                      ...
                      <div style={{ height: '6px', backgroundColor: '#E2E8F0', borderRadius: '3px', overflow: 'hidden' }}>
                        <div
                          style={{
                            height: '100%',
                            width: `${Math.min(100, (cat.val / cat.max) * 100)}%`,
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {(report.strengths || []).length > 0 ? (
                    ...
                    <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                      Valid machine-readable text and clean section separation detected.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="card" style={{ borderLeft: '4px solid var(--warning)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {((report.warnings || []).length > 0) ? (
                    ...
                    <div style={{ fontSize: '13px', color: 'var(--text-muted)' }}>
                      No critical layout or section format blockers found.
                    </div>
                  )}
                </div>
              </div>
            ...
            {((report.issues && report.issues.length > 0) || (report.suggestions && report.suggestions.length > 0)) && (
              <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <h3 style={{ fontSize: '16px', fontWeight: '700' }}>Identified ATS Issues & Suggestions</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                    ...
                    >
                      <div style={{ maxWidth: '80%' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                          ...
                          {iss.section && (
                            <span className="badge badge-secondary" style={{ fontSize: '11px', textTransform: 'capitalize' }}>
                              {iss.section}
                            </span>
                          )}
                        </div>
                        ...
                        {iss.suggestion && (
                          <div style={{ fontSize: '12.5px', color: 'var(--primary)', marginTop: '4px', fontWeight: '500' }}>
                      ...
                      <button
                        onClick={() => navigate(`/dashboard/builder/${selectedResumeId}`)}
                  ...
                  {(!report.issues || report.issues.length === 0) && (report.suggestions || []).map((sug, i) => (
                    <div
                      key={i}
                      style={{
                        padding: '12px 16px',
            ...
            {/* Actionable Recommendations from legacy fallback if present */}
            {(report.fixRecommendations || []).length > 0 && !report.issues && (
              <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            ...
            {/* Official ATS Compatibility Score Disclaimer */}
            <div style={{
              padding: '16px',
              backgroundColor: 'var(--bg-subtle)',
              borderRadius: '8px',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px', color: 'var(--text-primary)', fontWeight: '600' }}>
                ...
                <span>Notice on ATS Compatibility Scoring</span>
              </div>
              <p style={{ margin: 0 }}>
                <strong>Disclaimer:</strong> {report.disclaimer || "ATS Compatibility Score is an algorithmic diagnostic based on standard parser rules, section structures, and keyword density. It is not an official universal ATS score from any single enterprise hiring system."}
              </p>
            </div>
          </div>
        );
